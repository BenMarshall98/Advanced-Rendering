...
void Texture::Load(std::shared_ptr<DX::DeviceResources> pDeviceResources)
{
	...
	auto result = DirectX::CreateDDSTextureFromFile(device, temp.c_str(), nullptr, mTexture.ReleaseAndGetAddressOf());
}
...
void Texture::ReleaseTexture(std::shared_ptr<DX::DeviceResources> pDeviceResources, const unsigned int pIndex) const
{
	...
	context->PSSetShaderResources(pIndex, 0, nullTexture.GetAddressOf());
}

void Texture::UseDomainTexture(std::shared_ptr<DX::DeviceResources> pDeviceResources, unsigned int pIndex) const
{
	auto context = pDeviceResources->GetD3DDeviceContext();

	context->DSSetShaderResources(pIndex, 1, mTexture.GetAddressOf());
}

void Texture::ReleaseDomainTexture(std::shared_ptr<DX::DeviceResources> pDeviceResources, const unsigned int pIndex) const
{
	auto context = pDeviceResources->GetD3DDeviceContext();
